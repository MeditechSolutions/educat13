...
class JistiMeet(models.Model):
    ...
    internal_participants = fields.One2many('sinerkia_jitsi_meet.internal_user', 'meet', string='Internal Participant')
    ...
    def create(self, vals):
        ...
        vals['internal_participants'] = [(0,0,{'name':participante.partner_id.email}) for participante in self.env['res.users'].browse([x[1] for x in vals.get('participants', []) if x[0]==4])]
        if not vals['internal_participants'] :
            del vals['internal_participants']
        return super(JistiMeet, self).create(vals)
    
    def write(self, vals):
        if 'participants' in vals :
            vals['internal_participants'] = [(0,0,{'name':participante.partner_id.email}) for participante in self.env['res.users'].browse([x[1] for x in vals.get('participants', []) if x[0]==4])]
            if not vals['internal_participants'] :
                del vals['internal_participants']
        return super(JistiMeet, self).write(vals)
...
class JitsiMeetExternalParticipant(models.Model):
    ...
    def create(self, vals):
        ...
        if res.send_mail:
            ...
            correo_id = self.env['mail.template'].sudo().browse(template.id).send_mail(res.id)
            self.env['mail.mail'].browse(correo_id).write({'author_id':2})
            self.env['mail.mail'].browse(correo_id).send()
    ...
    def write(self, vals):
        if 'send_mail' in vals and vals.get('send_mail'):
            if not self.mail_sent:
                ...
                correo_id = self.env['mail.template'].sudo().browse(template.id).send_mail(self.id)
                self.env['mail.mail'].browse(correo_id).write({'author_id':2})
                self.env['mail.mail'].browse(correo_id).send()
...
class JitsiMeetInternalParticipant(models.Model):
    _name = 'sinerkia_jitsi_meet.internal_user'
    _description = 'Jitsi Meeting Internal Participant'
